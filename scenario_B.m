function [data, data_tr, data_te, val_set] = scenario_B(data, val_set)

    % In this fucntion we preprocess the data for the scenario B, i.e., we
    ...
    for i = 1:data.num
        if data.targets(i,1) == 1 || data.targets(i,2) == 1
            data.y(i) = 2;
        else
            data.y(i) = 1;
        end
    ...
    for i = 1:val_set.num
        if val_set.targets(i,1) == 1 || val_set.targets(i,2) == 1
            val_set.y(i) = 2;
        else
            val_set.y(i) = 1;
        end
